...
class LineBox(object):
    ...
    def __init__(self):
        #----  Publisher  -----------------
        self._lotate_pub = rospy.Publisher('cmd_vel', Twist, queue_size=1)
    ...
    ####  OPENCV PROCESSING  ####################################################
        ...
        for depth in range(8):
            ...
            #image,contours,hierarchy = cv2.findContours(mask_black_trim, cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)[0]

            contours = cv2.findContours(mask_black_trim, cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)[0]
            for i in range(len(contours)):
                ...
                #---- choose rect ---------------
                if diagonal<100 and sort_y[0][1]-30>0 and mask_white[sort_y[0][1]-70, sort_y[0][0]]==white_pixel and sort_y[0][1]>260 and ang<10 and diagonal>20 and diagonal_h<90:
                    #print diagonal
        ...
        #---- line ang --------------------------------------
            ...
            if theta > 2:
                self._lotate.angular.z = 0.5
                self._lotate.linear.x = 0.0

            elif theta < -2:
                self._lotate.angular.z = -0.5
                self._lotate.linear.x = 0.0

            elif (theta < 2) and (theta > -3) :
                self._lotate.linear.x = 0.5
                self._lotate.angular.z = 0.0

            elif (theta > 4) or (theta <4):
                self._lotate.angular.z = 0.0
                self._lotate.linear.x = 0.0
            else:
                self._lotate.angular.z = 0.0
